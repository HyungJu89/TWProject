...
function MainBanner({ channelId }) {
        ...
        axios.get('/channelRest/live/' + channelId).then((a) =>
            {
                ...
                return (a.data.content.topExposedVideos.openLive)}
        )
    )
    ...
    return (
        <div className={style.MainBanner}>
            {channe.isLoading ? <div  className={style.liveImage}>로딩중</div> : channe.isError ? <div  className={style.liveImage}>에러남</div> : <div className={style.liveImage}><img src={channe.data.liveImageUrl.replace("{type}", 1080)} /></div>}
